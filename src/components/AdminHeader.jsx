...
export function AdminHeader({ restaurantName, onLogout, callRequests = [] }) {
  return (
    <header className="h-16 bg-card border-b border-border px-6 flex items-center justify-between">
      <div className="flex items-center gap-4">
        {/* Store Name and Message */}
        <div className="text-center flex-1 lg:flex-none">
          <h1 className="text-lg font-bold text-foreground">{restaurantName || '내 매장'}</h1>
      ...
      <div className="flex items-center gap-4">
          ...
          <DropdownMenuContent align="end" className="w-80">
            ...
            {callRequests.length > 0 ? (
              callRequests.map((call, index) => (
                <DropdownMenuItem key={call.id}>
                  <div className="flex flex-col gap-1">
                    <p className="text-sm font-medium">테이블 {call.tableNumber} 호출</p>
                    <p className="text-xs text-muted-foreground">
                      {call.message || '고객이 호출했습니다'}
                    </p>
                  </div>
                </DropdownMenuItem>
              ))
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuLabel>사장님</DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem>
