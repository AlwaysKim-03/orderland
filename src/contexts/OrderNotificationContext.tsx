...
export function OrderNotificationProvider({ children }: { children: ReactNode }) {
  ...
  // Firebase 실시간 신규 주문 감지 - 로그인 상태에서만 활성화
  useEffect(() => {
    // 로그아웃 상태면 실시간 감지 비활성화
    if (!user) {
      console.log('로그아웃 상태 - 실시간 주문 감지 비활성화');
      return;
    }

    console.log('로그인 상태 - 실시간 신규 주문 감지 시작...');
    ...
    return () => {
      if (unsubscribe) {
        console.log('실시간 주문 감지 리스너 정리');
        unsubscribe();
      }
    };
  }, [user]); // user를 의존성으로 추가하여 로그인 상태 변경 시 리스너 재설정
...
export function useOrderNotification() {
  ...
  if (context === undefined) {
    throw new Error('useOrderNotification must be used within a OrderNotificationProvider');
  }
  return context;
} 